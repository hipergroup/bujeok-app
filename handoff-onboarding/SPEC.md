...
**위에서 아래 순서:** 제목 → 생년월일 → **띠 미리보기** → 시진 → 성별 → 이름 → CTA.
띠 미리보기가 생년월일 바로 다음에 오는 것이 이번 변경점이다.
...
### 1-3. 띠 미리보기 — **생년월일 카드 바로 아래**
`margin-top:14px`, 테두리 `rgba(143,107,20,.35)`, 바탕 `rgba(255,250,236,.55)`, 12px 라운드, `padding:14px 16px`, 가로 flex gap 14px.
...
**위치가 중요하다.** 날짜를 넣은 즉시 그 자리에서 띠가 확인되어야 한다.
카드 맨 아래에 두면 결과가 입력에서 멀어져 스크롤해야 보인다. 반드시 생년월일 카드 다음에 온다.
...
**네 칸의 높이가 반드시 같아야 한다.** 아래 세 가지를 지키면 어긋나지 않는다.

1. 박스 높이를 `height:72px` + `box-sizing:border-box` 로 **고정**한다. `padding` 으로 높이를 만들지 않는다.
2. 테두리는 `border` 가 아니라 **`box-shadow: inset`** 으로 그린다.
   `border` 는 박스 크기를 밀어내므로 굵기가 다른 칸이 생기면 높이가 어긋난다.
3. 박스 아래 담당 영역 텍스트는 `height:24px` 로 두 줄 자리를 고정한다.
   `나 자신·배우자` 처럼 긴 글이 오는 칸만 아래로 밀리는 것을 막는다.

- 각 칸 박스: 8px 라운드, `height:72px`, 세로 가운데 정렬
  - 선택된 칸: 바탕 `rgba(167,43,33,.09)`, `box-shadow: inset 0 0 0 1.5px rgba(167,43,33,.45)`
  - 나머지: 바탕 `rgba(122,74,52,.04)`, `box-shadow: inset 0 0 0 1px rgba(122,74,52,.18)`
  - 천간 20px → 18×1px 선 → 지지 20px, 각각 `OHENG_COLORS`
- 박스 아래: 기둥명 11px bold (일주는 `#A72B21`) → `나 자신` 알약 → 담당 영역 9px `rgba(122,74,52,.6)`
  - `나 자신` 알약은 **박스 밖 아래**에 놓는다. 9px, `#A72B21` 바탕, `#F6EDD9` 글자, `padding:2px 7px`.
    자리는 `height:15px` 로 고정해 다른 세 칸도 같은 높이를 갖게 한다.
  - **박스 위에 겹쳐 띄우지 말 것.** 테두리를 덮고 칸 높이를 어긋나게 만든다.
- **바깥 래퍼에 테두리·바탕을 주지 않는다.** 일주는 강조 칸이면서 기본 선택 칸이라
  래퍼까지 칠하면 틀이 이중으로 겹쳐 더 커 보인다. 선택 표시는 박스 하나로만 한다.
